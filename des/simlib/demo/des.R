...
###################################################################################################################
# Function to serve as both an arrival event of a job to the system, as well as 
# the non-event of a job's arriving to a subsequent station along its route.
arrive <- function(env, newjob) {
    # If this is a new arrival to the system, then 
    # generate the time of the next arrival and 
    # determine the job type and task number of the arriving job.
    if (newjob == 1) {
        event_schedule(env, env$des$get_simtime() + rexp(1, 1.0/env$mean_interarrival), env$EVENT_ARRIVAL)
        env$job_type <- sample(seq(env$MAX_NUM_JOB_TYPES), 1L, prob=env$prob_distrib_job_type)
        env$task     <- 1L
    }
    # Determine the station from the route matrix
    with(env, station <- route[[job_type]][task])
    ...
    if (with(env, num_machines_busy[station] == num_machines[station])) {
        ...
        #  2. Job type.
        #  3. Current task number
        transfer <- c(NA, env$des$get_simtime(), env$job_type, env$task)
        env$des$insert(env$station, transfer, TRUE, 2L)
    } else {
        # A machine in this station is idle, so start service on the arriving job (which has a delay of zero)
        # with(env, des$sampst(0.0, station))
        # with(env, des$sampst(0.0, MAX_NUM_STATIONS + job_type))
        with(env, num_machines_busy[station] <- num_machines_busy[station] + 1L)
        # with(env, des$timest(1.0*num_machines_busy[station], station))
        # Schedule a service completion. Note defining attributes beyond the first two 
        # for the event record before invoking event_schedule
        ...
        with(env, des$set_transfer(4L, task))
        event_schedule(env, 
            env$des$get_simtime() + rgamma(1, shape=2, rate=with(env, 1.0/mean_service[[job_type]][task])),
            env$EVENT_DEPARTURE)
    }
}
...
depart <- function(env) {
    ...
    if (with(env, des$get_listsize(station) == 0)) {
        ...
        job_type_queue <- env$des$get_transfer(2L);
        task_queue     <- env$des$get_transfer(3L);
        # env$des$sampst(env$des$get_simtime() - env$des$get_transfer(1L), env$MAX_NUM_STATIONS + job_type_queue)
        # Schedule end of service for this job at this station. 
        # Note defining attributes beyond the first two for the event record before invoking event_schedule.
        env$des$set_transfer(3L, job_type_queue)
        env$des$set_transfer(4L, task_queue)
        event_schedule(env, 
            env$des$get_simtime() + rgamma(1, shape=2, rate=1.0/env$mean_service[[job_type_queue]][task_queue]),
            env$EVENT_DEPARTURE)
    }
    # If the current departing job has one or more tasks yet to be done, 
    # send the job to the next station on its route
    if (env$task < length(env$route[[env$job_type]])) {
        with(env, task <- task + 1L)
        arrive(env, 2L)
    }
}

# Auxiliary env->event_schedule (same as env->des->event_schedule)
event_schedule <- function(env, time_of_event, type_of_event) {
    env$des$set_transfer(1L, time_of_event)
    env$des$set_transfer(2L, type_of_event)
    env$des$insert(10L, TRUE, 0)
}

# Auxiliary env->timing (same as env->des->timing)
timing <- function(env) {
  env$des$remove(10L, TRUE)
  if (env$des$get_transfer(1L) < env$des$get_simtime()) {
    print("Event time precedes the simulation clock.")
    return(-1L)
  }
  # Advance the simulation clock and set the next event type
  env$des$set_simtime(env$des$get_transfer(1L))
  env$des$set_next_event_type(env$des$get_transfer(2L))
}
